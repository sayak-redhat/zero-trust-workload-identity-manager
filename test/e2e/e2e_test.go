	...
	Context("SpireAgent attestation", func() {
		...
		It("should have SPIFFE ID matching expected URI SAN", Label("reconciliation"), func() {
			...
			Expect(spiffeID).To(ContainSubstring("/ns/" + attestationTestNamespace + "/sa/" + attestationTestSA),
				"SPIFFE ID should match the ClusterSPIFFEID template")
		...
		It("should rotate SVID before expiry", Label("reconciliation"), func() {
			...
			Eventually(func() bool {
				...
				if err != nil {
					fmt.Fprintf(GinkgoWriter, "failed to read svid.pem: %v\n", err)
					return false
				}
				newCert, err := utils.ParseCertificatePEM([]byte(newPEM))
				if err != nil {
					fmt.Fprintf(GinkgoWriter, "failed to parse rotated svid.pem: %v\n", err)
					return false
				}
		...
		It("should NOT issue SVID to pod without matching ClusterSPIFFEID", Label("negative-input-validation"), func() {
			negativeNS := &corev1.Namespace{
				ObjectMeta: metav1.ObjectMeta{
					...
					Labels: map[string]string{
						"kubernetes.io/metadata.name": utils.AttestationNegativeNamespace,
					},
				},
			}
			By("Creating negative test namespace (no ClusterSPIFFEID matches this namespace)")
			err := k8sClient.Create(testCtx, negativeNS)
			Expect(err).NotTo(HaveOccurred())
			DeferCleanup(func(ctx context.Context) {
			...
			cm := &corev1.ConfigMap{
				ObjectMeta: metav1.ObjectMeta{
					Name:      utils.SpiffeHelperConfigMapName,
					Namespace: utils.AttestationNegativeNamespace,
				},
			...
			negativePod := &corev1.Pod{
				ObjectMeta: metav1.ObjectMeta{
					Name:      utils.AttestationNegativePodName,
					Namespace: utils.AttestationNegativeNamespace,
			...
			By("Verifying that svid.pem is NOT created (no matching ClusterSPIFFEID)")
		...
		It("should propagate ClusterSPIFFEID updates to workload", Label("reconciliation"), func() {
			...
			By("Updating ClusterSPIFFEID to use a modified template with extra path segment")
			...
			DeferCleanup(func(ctx context.Context) {
				By("Restoring original ClusterSPIFFEID template")
			...
			By("Waiting for the workload to receive an SVID with the updated SPIFFE ID")
			Eventually(func() bool {
				newPEM, err := utils.ReadFileFromPod(testCtx, attestationTestNamespace, attestationTestPodName, attestationTestAppContainer, utils.AttestationCertPath+"/svid.pem")
				if err != nil {
					return false
				...
				"workload should receive an SVID with updated SPIFFE ID")
		})
		...
		It("should re-attest and issue fresh SVID after pod recreation", Label("reconciliation"), func() {
			...
			By("Re-creating the attestation pod with same spec")
			readOnlyTrue := true
			helperConf := utils.DefaultAttestationSpiffeHelperConfig().String()
			cm := &corev1.ConfigMap{}
			...
			Eventually(func() bool {
				...
				newSerial := freshCert.SerialNumber.String()
				if newSerial != oldSerial {
					fmt.Fprintf(GinkgoWriter, "Re-attested SVID serial: %s (was %s)\n", newSerial, oldSerial)
					return true
				}
				return false
			}).WithTimeout(utils.DefaultTimeout).WithPolling(utils.DefaultInterval).Should(BeTrue(),
				"re-created pod should receive a fresh SVID with a different serial number")
		})
		...
		It("should issue distinct SPIFFE IDs to different ServiceAccounts", Label("reconciliation"), func() {
			...
			DeferCleanup(func(ctx context.Context) {
				_ = k8sClient.Delete(ctx, sa2)
			})

			By("Creating a second ClusterSPIFFEID matching the second pod")
			...
			DeferCleanup(func(ctx context.Context) {
				_ = k8sClient.Delete(ctx, clusterSPIFFEID2)
			})

			By("Creating a second attestation pod with the different ServiceAccount")
			readOnlyTrue := true
			helperConf := utils.DefaultAttestationSpiffeHelperConfig().String()
		...
		It("should issue bundle.pem as a valid CA certificate", Label("security-context"), func() {
			...
			for _, ca := range caCerts {
				if ca.IsCA && (ca.KeyUsage&x509.KeyUsageCertSign != 0) {
					foundCA = true
					fmt.Fprintf(GinkgoWriter, "CA cert: Subject=%s, IsCA=%v, KeyUsage=%v\n", ca.Subject, ca.IsCA, ca.KeyUsage)
					break
				}
			}
